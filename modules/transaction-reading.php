...
/**
 * Auto-sync bank transactions to journal entries
 * Runs automatically on page load
 */
function syncBankTransactionsAuto() {
    global $conn;
    
    try {
        $currentUser = getCurrentUser();
        $limit = 100; // Sync up to 100 transactions per page load
        $syncedCount = 0;
        
        // Check which table exists: bank_transactions or transactions
        $tableCheck = $conn->query("SHOW TABLES LIKE 'bank_transactions'");
        $useBankTransactions = ($tableCheck && $tableCheck->num_rows > 0);
        
        if (!$useBankTransactions) {
            $tableCheck = $conn->query("SHOW TABLES LIKE 'transactions'");
            if (!$tableCheck || $tableCheck->num_rows === 0) {
                return; // Neither table exists, skip sync
            }
        }
        
        $tableName = $useBankTransactions ? 'bank_transactions' : 'transactions';
        
        // Get unsynced bank transactions
        $sql = "SELECT bt.*, tt.type_name
                FROM {$tableName} bt
                INNER JOIN transaction_types tt ON bt.transaction_type_id = tt.transaction_type_id
                WHERE NOT EXISTS (
                    SELECT 1 FROM journal_entries je 
                    WHERE je.reference_no = COALESCE(bt.transaction_ref, CONCAT('TXN-', bt.transaction_id))
                )
        ...
        $fiscalPeriodId = $fiscalPeriod['id'] ?? 1;
        
        // Get journal type
        $journalTypeSql = "SELECT id FROM journal_types WHERE code = 'CR' LIMIT 1";
        $journalTypeResult = $conn->query($journalTypeSql);
        $journalType = $journalTypeResult->fetch_assoc();
        $journalTypeId = $journalType['id'] ?? 1;
        
        // Account mappings
        $accountMappings = [
            1 => ['debit' => '1002', 'credit' => '2601'], // Deposit
            2 => ['debit' => '2601', 'credit' => '1002'], // Withdrawal
            3 => ['debit' => '1002', 'credit' => '1002'], // Transfer Out
            4 => ['debit' => '1002', 'credit' => '1002'], // Transfer In
            5 => ['debit' => '1002', 'credit' => '5101'], // Interest
            6 => ['debit' => '1002', 'credit' => '1102'], // Loan Payment
            7 => ['debit' => '8002', 'credit' => '1002'], // Fee
            8 => ['debit' => '1002', 'credit' => '1002'], // Transfer
        ];
        
        while ($bankTx = $result->fetch_assoc()) {
            try {
                $transactionTypeId = $bankTx['transaction_type_id'];
                $amount = floatval($bankTx['amount']);
                $transactionRef = $bankTx['transaction_ref'] ?? 'TXN-' . $bankTx['transaction_id'];
                $description = $bankTx['description'] ?? "Bank Transaction: {$bankTx['type_name']}";
                $transactionDate = date('Y-m-d', strtotime($bankTx['created_at']));
                $typeName = $bankTx['type_name'] ?? '';
                
                // Get account mapping
                $mapping = $accountMappings[$transactionTypeId] ?? ['debit' => '1002', 'credit' => '5001'];
                
                // Get account IDs
                $debitAccountSql = "SELECT id FROM accounts WHERE code = ? LIMIT 1";
                $creditAccountSql = "SELECT id FROM accounts WHERE code = ? LIMIT 1";
                
                $stmtDebit = $conn->prepare($debitAccountSql);
                $stmtDebit->bind_param('s', $mapping['debit']);
                $stmtDebit->execute();
                $debitAccount = $stmtDebit->get_result()->fetch_assoc();
                
                $stmtCredit = $conn->prepare($creditAccountSql);
                $stmtCredit->bind_param('s', $mapping['credit']);
                $stmtCredit->execute();
                $creditAccount = $stmtCredit->get_result()->fetch_assoc();
                
                // Fallback to default accounts if not found
                if (!$debitAccount) {
                    $stmtDebit->bind_param('s', '1002');
                    $stmtDebit->execute();
                    $debitAccount = $stmtDebit->get_result()->fetch_assoc();
                }
                if (!$creditAccount) {
                    $stmtCredit->bind_param('s', '5001');
                    $stmtCredit->execute();
                    $creditAccount = $stmtCredit->get_result()->fetch_assoc();
                }
                
                if (!$debitAccount || !$creditAccount) {
                    continue; // Skip if accounts don't exist
                }
                ...
                $journalNo = 'JE-BT-' . date('Ymd', strtotime($transactionDate)) . '-' . str_pad($syncedCount + 1, 4, '0', STR_PAD_LEFT);
                
                // Create journal entry
                $jeSql = "INSERT INTO journal_entries (
                ...
                $memo = "Bank Transaction: {$typeName} - {$description}";
                
                // Debit line
                $lineSql = "INSERT INTO journal_lines (journal_entry_id, account_id, debit, credit, memo) VALUES (?, ?, ?, 0, ?)";
                $stmtLine = $conn->prepare($lineSql);
                $stmtLine->bind_param('iids', $journalEntryId, $debitAccountId, $amount, $memo);
                $stmtLine->execute();
                
                // Credit line
                $lineSql2 = "INSERT INTO journal_lines (journal_entry_id, account_id, debit, credit, memo) VALUES (?, ?, 0, ?, ?)";
                $stmtLine2 = $conn->prepare($lineSql2);
                $stmtLine2->bind_param('iids', $journalEntryId, $creditAccountId, $amount, $memo);
                $stmtLine2->execute();
                
                $syncedCount++;
                
            } catch (Exception $e) {
                error_log("Error syncing transaction: " . $e->getMessage());
                continue;
            }
        }
        
        if ($syncedCount > 0) {
            error_log("Auto-synced {$syncedCount} bank transaction(s) to journal entries");
        }
        
    } catch (Exception $e) {
        error_log("Auto-sync bank transactions error: " . $e->getMessage());
    }
}
...
// Auto-sync bank transactions before fetching
try {
    syncBankTransactionsAuto();
} catch (Exception $e) {
